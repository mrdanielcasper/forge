...
traverse(ast, {
  ...
  JSXAttribute(path) {
    if (path.node.name.name === 'style') {
      console.error(`❌ AST Violation: Inline 'style' object detected at line ${path.node.loc.start.line}. Use Tailwind CSS classes exclusively.`);
      hasError = true;
    }
  },
  ...
  JSXOpeningElement(path) {
    if (path.node.name.name === 'img') {
      ...
      if (!hasAlt) {
        console.error(`❌ AST Violation: <img> tag missing 'alt' attribute at line ${path.node.loc.start.line}.`);
        hasError = true;
      }
    }
  }
